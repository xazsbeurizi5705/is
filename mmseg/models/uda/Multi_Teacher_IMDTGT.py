            ...
            if self.local_iter % 2 == 0:
                ...
                for i in range(batch_size):
                    ...
                    src_imd_mixed_img[i], src_imd_mixed_lbl[i] = strong_transform(
                        strong_parameters,
                        data=torch.stack((img[i], imd_img[i])),  # 源域和中间域的图片混合
                        target=torch.stack((gt_semantic_seg[i][0], imd_pseudo_label[i])))  # 源域和中间域标签混合
                    _, imd_pseudo_weight[i] = strong_transform(
                        strong_parameters,
                        target=torch.stack((gt_pixel_weight[i], imd_pseudo_weight[i])))  # 源域和中间域的权重混合
