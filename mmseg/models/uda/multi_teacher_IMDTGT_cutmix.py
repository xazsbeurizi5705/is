...
#定义一个cutmix的函数
def cutmix(image1, label1, image2, label2, alpha):
    lam = Beta(alpha, alpha).sample().to(image1.device)
    batch_size = image1.size(0)
    idx = torch.randperm(batch_size)
    mixed_image = lam * image1 + (1 - lam) * image2
    mixed_label = lam * label1 + (1 - lam) * label2
    mixed_image[idx] = mixed_image
    mixed_label[idx] = mixed_label
        ...
        if self.local_iter > 0:
            ...
            if self.local_iter % 2 == 0:
                ...
                for i in range(batch_size):
                    ...
                    src_imd_mixed_img[i], src_imd_mixed_lbl[i] = cutmix(img[i], gt_semantic_seg[i][0], imd_img[i],
                                                                        imd_pseudo_label[i], alpha=0.4)# 源域和中间域标签混合
                    _, imd_pseudo_weight[i] = strong_transform(strong_parameters, target=torch.stack(
                        (gt_pixel_weight[i], imd_pseudo_weight[i]))) # 源域和中间域的权重混合
                src_imd_mixed_img = torch.cat(src_imd_mixed_img)
                src_imd_mixed_lbl = torch.cat(src_imd_mixed_lbl)  # dim=0

                # Train on mixed images by teacher_imd
                src_imd_mix_losses = self.get_model().forward_train(
                    src_imd_mixed_img, img_metas, src_imd_mixed_lbl, imd_pseudo_weight, return_feat=True)  # 源域和中间域loss
            ...
            if self.local_iter % 2 == 1:  # Learn from teacher_targer as iteration is 2k + 1
                ...
                for i in range(batch_size):
                    ...
                    mixed_img[i], mixed_lbl[i] = cutmix(img[i],  gt_semantic_seg[i][0],target_img[i], pseudo_label[i],alpha=0.4)
                ...
                if self.local_iter % self.debug_img_interval == 0:
                    ...
                    for j in range(batch_size):
                        ...
                        subplotimg(
                            ...
                            #            cmap="cityscapes")
                        subplotimg(
                                axs[1][3], mixed_lbl[j], 'Seg Target', cmap='cityscapes')
                        subplotimg(
